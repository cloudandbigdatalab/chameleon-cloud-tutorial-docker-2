...
assuming the Dockerfile for server is in the server folder within the current directory. Note for the db container we're using the unmodified Postgres image off Docker Hub so their isn't a folder for it. Here's a quick explanation of what's going on with our composition.
